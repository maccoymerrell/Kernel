...
#define CONSOLE_START_ADDRESS (unsigned char *)0xb8000
...
/* -- GLOBAL VARIABLES -- */
 ...
 int Console::csr_y;
 unsigned char * Console::textmemptr; /* text pointer */
    ...
    *  backcolor too */
    ...
    if(csr_y >= 25)
    {
        ...
        *  back in the buffer by a line */
        ...
        memcpy ((char*)CONSOLE_START_ADDRESS, (char*)CONSOLE_START_ADDRESS + temp * 80, (25 - temp) * 80 * 2);

        /* Finally, we set the chunk of memory that occupies
        *  the last line of text to our 'blank' character */
        memsetw (CONSOLE_START_ADDRESS + (2*((25 - temp) * 80)), blank, 80);
        csr_y = 25 - 1;
    }
}
...
void Console::cls() {
    ...
    *  color */
    for(int i = 0; i < 25; i++) 
        memsetw (CONSOLE_START_ADDRESS + (2*(i * 80)), blank, 80);
...
void Console::putch(const char _c){
    ...
    *  Index = [(y * width) + x] */
    ...
    {
        //*((int*)CONSOLE_START_ADDRESS)=0x07690748;
        unsigned char * where = CONSOLE_START_ADDRESS + (2*(csr_y * 80 + csr_x));
